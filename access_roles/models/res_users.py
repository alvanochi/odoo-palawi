...
class ResUsers(models.Model):
    ...
    def create(self, vals_list):
        ...
        for user in users:
            if user.access_role_id:
                user.write({
                    'groups_id': [Command.set(user.access_role_id.groups_ids.ids)]
                })
        return users
    ...
    def write(self, vals):
        ...
        # Capture old roles before update for syncing user_ids
        old_roles_map = {}
        if 'access_role_id' in vals:
            for user in self:
                old_roles_map[user.id] = user.access_role_id

        result = super(ResUsers, self).write(vals)

        if 'access_role_id' in vals:
            new_role_id = vals['access_role_id']
            
            # Sync user_ids on Access Role
            for user in self:
                old_role = old_roles_map.get(user.id)
                # Remove from old role if changed
                if old_role and old_role.id != new_role_id:
                    old_role.sudo().write({'user_ids': [Command.unlink(user.id)]})
                
                # Add to new role
                if new_role_id:
                     self.env['access.role'].browse(new_role_id).sudo().write({'user_ids': [Command.link(user.id)]})
